                ...
                return when (expression) {
                    is IrReturnableBlock -> {
                        ...
                        val transformedReturnableBlock = with(expression) {
                            IrReturnableBlockImpl(startOffset, endOffset, if (someoneFolded) coercion.type else expression.type, newSymbol,
                                    origin, statements, sourceFileName)
                        }
                        transformedReturnableBlock.transformChildrenVoid(object: IrElementTransformerVoid() {
                            override fun visitExpression(expression: IrExpression): IrExpression {
                                foldedReturnableBlockValues[expression]?.let {
                                    return if (someoneFolded)
                                        it.getFullExpression(coercion, cast)
                                    else
                                        it.expression
                                }
                            ...
                            override fun visitReturn(expression: IrReturn): IrExpression {
                                expression.transformChildrenVoid(this)
                                if (expression.returnTargetSymbol != oldSymbol)
                                    return expression
                                with(expression) {
                                    return IrReturnImpl(startOffset, endOffset, context.irBuiltIns.nothingType, newSymbol, value)
                                }
                            }
                        })
                        ...
                        val transformedBlock =
                                with(expression) {
                                    IrBlockImpl(startOffset, endOffset,
                                            if (foldedLastStatement.folded) coercion.type else expression.type,
                                            origin, statements)
                                }
                            ...
                            if (expression.typeOperand.getInlinedClass() != coercionDeclaringClass)
                                ...
                                if (foldedArgument.folded)
                                    ...
                                    PossiblyFoldedExpression(
                                            IrTypeOperatorCallImpl(startOffset, endOffset, type, operator, typeOperand,
                                                    typeOperandClassifier, foldedArgument.expression),
                                            false
                                    )
                                }
                            }
